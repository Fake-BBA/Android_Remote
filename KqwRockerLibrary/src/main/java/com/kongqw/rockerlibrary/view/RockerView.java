 ...
 * extend by BBA on 2018/6/7(401131019@qq.com) from ZSC university
 * 摇杆控件
 */
public class RockerView extends View {
    private static final String TAG = "RockerView";
    final String LOG_TAG = "RockerView";   //日志类型
    ...
    private OnCoordinateChangeListener mOnCoordinateChangeListener;
    ...
    private Point getRockerPositionPoint(Point centerPoint, Point touchPoint, float regionRadius, float rockerRadius) {
        ...
        // 回调 返回参数
        int regionX=Math.abs((int)lenX)<(int)regionRadius?(int)lenX:(int)regionRadius;
        int regionY=Math.abs((int)lenY)<(int)regionRadius?(int)lenY:(int)regionRadius;


        Logger.i(TAG, "getRockerPositionPoint: 角度 :" + angle);
        if (lenXY + rockerRadius <= regionRadius) { // 触摸位置在可活动范围内
            callBack(angle,new Point(touchPoint.x-centerPoint.x,touchPoint.y-centerPoint.y));
            return touchPoint;
        } else { // 触摸位置在可活动范围以外
            // 计算要显示的位置
            ...
            callBack(angle,new Point(showPointX-centerPoint.x, showPointY-centerPoint.y));
    ...
    private void callBackStart() {
        ...
        if(null!=mOnCoordinateChangeListener)
        {
            mOnCoordinateChangeListener.onStart();
        }
     ...
     *
     * @param angle 摇动角度
     * @param point 摇杆离圆心X,Y轴距离
     */
    private void callBack(double angle,Point point) {
        if (null != mOnAngleChangeListener) {
            mOnAngleChangeListener.angle(angle);
        }
        if(null!=mOnCoordinateChangeListener)
        {
            mOnCoordinateChangeListener.Coordinate(point);
        }
        if (null != mOnShakeListener) {
    ...
    private void callBackFinish() {
        ...
        if(null!=mOnCoordinateChangeListener)
        {
            mOnCoordinateChangeListener.onFinish();
        }
        if (null != mOnShakeListener) {
     ...
     * @param listener 回调接口
     */
    public void setOnCoordinateChangeListener(OnCoordinateChangeListener listener) {
        mOnCoordinateChangeListener = listener;
    }
